...
const Store = {
  ...
  activity(limit) {
    if (this.live) {
      ...
      // `created` is the home agents' field name; `created_at` is what the
      // petcare agent writes. Fallback only — verticals that have `created`
      // never reach the last term.
      src.sort((a, b) => String(b.updated || b.created || b.created_at || '')
        .localeCompare(String(a.updated || a.created || a.created_at || '')));
  ...
  _liveActivity(r) {
    ...
    return {
      ...
      who: r.source || (ai ? 'AI agent' : ''), t: r.updated || r.created || r.created_at,
    };
  },
  pushActivity(ev) {
...
function nowTime() { return fmtTime(new Date(), { second: undefined }); }
...
// Language mark for a call/record. The voice bridge stores `language` (si/ta/en)
// on each AI booking; manual records have none → show a dash.
function langBadge(code) {
  const m = { si: ['සිංහල', 'emerald'], en: ['English', 'sky'], ta: ['தமிழ்', 'amber'] };
  const x = m[code];
  return x ? `<span class="px-2 py-0.5 rounded-md text-[11px] border font-medium whitespace-nowrap badge-${x[1]}">${x[0]}</span>` : '<span class="text-muted-fg text-xs">—</span>';
}
...
const App = {
  ...
  async boot(id) {
    ...
    if (cfg.live) this._applyLiveChrome(); else this._applyDemoChrome();
    this.renderTabs();
    this.renderSideNav();
  ...
  _applyLiveChrome() {
    ...
    if (b) {
      ...
      b.innerHTML = '<i data-lucide="radio" class="w-3.5 h-3.5 shrink-0"></i><span>' + (this.cfg.liveBanner ||
        '<strong>Live data</strong> — records are captured from real phone calls and staff entries in real time; the reference catalog (doctors, branches) is editable demo data. This view auto-refreshes.') + '</span>';
      if (window.lucide) lucide.createIcons();
    }
    const rb = document.getElementById('btn-reset'); if (rb) rb.remove();
  },
  // The banner ships empty and is filled here once we know which mode we are in.
  // It used to hard-code the demo text in the template, which flashed a false
  // "no live database connected" claim on every load of a live dashboard.
  _applyDemoChrome() {
    const b = document.getElementById('demo-banner');
    if (!b) return;
    b.className = 'px-4 py-2 rounded-md text-xs bg-amber-bg text-amber-fg border border-amber-fg/20 flex items-center gap-2';
    b.innerHTML = '<i data-lucide="flask-conical" class="w-3.5 h-3.5 shrink-0"></i><span>Demo workspace — running on local dummy data (<code>static/data/' +
      esc(this.cfg.id) + '.js</code>). No live database connected; changes persist in this browser only.</span>';
    if (window.lucide) lucide.createIcons();
  },
  // Refresh cadence. Verticals that need near-instant feedback (the restaurant
  // order board) set cfg.pollMs lower; the default stays 4s for everyone else.
  _startPolling() {
    if (this._poll) clearInterval(this._poll);
    this._poll = setInterval(() => this.refreshLive(), this.cfg.pollMs || 4000);
  },
  // POST to a vertical-specific endpoint under /api/dash/<id>/… and refresh.
  // Used by configs that need more than the generic record PATCH (e.g. the
  // restaurant's status / rider / priority routes, which validate server-side).
  call(path, body) {
    return Store._post(path, body).then((r) => { this._scheduleLiveRefresh(); return r; });
  },
  async refreshLive() {
    if (!Store.live) return;
    if (this._calling) return;   // a re-render mid-batch would destroy the Stop button
  ...
  /* In DEMO mode this queue plays a scripted call and invents an outcome.
   * In LIVE mode it must never do that: Store.update() POSTs to the backend, so
   * a coin-flip here would cancel somebody's real booking. A vertical that can
   * genuinely dial declares `liveCall(r, App)` on its callqueue tab (sales does,
   * via /orders/<id>/confirm-call); the outcome is then written server-side when
   * the call actually ends. A vertical without one gets a disabled, honest UI. */
  _canCall(t) { return !Store.live || typeof t.liveCall === 'function'; },
  view_callqueue(t) {
    const q = this._queueRows(t);
    const counters = this._callCounters(t);
    const canCall = this._canCall(t);
    const controls = canCall
      ...
      : `<button disabled title="Outbound calling is not connected for this dashboard" class="px-3 py-1.5 border border-border rounded-md text-sm text-muted-fg opacity-60 cursor-not-allowed flex items-center gap-1.5"><i data-lucide="phone-off" class="w-3.5 h-3.5"></i> Start AI calls</button>`;
    const notice = canCall ? '' :
      `<div class="px-5 py-2.5 border-b border-border bg-amber-bg/40 text-amber-fg text-[11px] flex items-start gap-2">
        <i data-lucide="alert-triangle" class="w-3.5 h-3.5 shrink-0 mt-0.5"></i>
        <span>Outbound calling is <strong>not connected</strong> for this dashboard, so this queue cannot place calls or set an outcome. Records below update only when the AI agent handles a real call, or when staff confirm/cancel them on the ${esc(t.recordsTabLabel || 'records')} tab.</span>
      </div>`;
    return `
  ...
  _queueListHtml(t, q) {
    ...
    const rowBtn = (r) => canCall
      ...
      : `<button disabled title="Outbound calling is not connected for this dashboard" class="text-xs px-2.5 py-1 rounded-md border border-border text-muted-fg opacity-50 cursor-not-allowed flex items-center gap-1"><i data-lucide="phone-off" class="w-3 h-3"></i> Call now</button>`;
    return q.map(r => `
  ...
  async startCalls(tabId) {
    ...
    if (!this._canCall(t)) { toast('Outbound calling is not connected for this dashboard', 'error'); return; }
    const q = this._queueRows(t);
    if (!q.length) { toast('Queue is empty'); return; }
    if (Store.live && !confirm(`Place ${q.length} REAL phone call${q.length === 1 ? '' : 's'} now? The AI agent will ring every contact left in this queue.`)) return;
    this._calling = true;
    const sb0 = document.getElementById('cq-start'), st0 = document.getElementById('cq-stop'), lv0 = document.getElementById('cq-live');
    if (sb0) sb0.classList.add('hidden'); if (st0) st0.classList.remove('hidden'); if (lv0) lv0.classList.remove('hidden');
    for (const r of q) {
      if (!this._calling) break;
      await this._runCall(t, r);
    }
    this._calling = false;
  ...
  async callOne(tabId, recId) {
    ...
    if (!this._canCall(t)) { toast('Outbound calling is not connected for this dashboard', 'error'); return; }
    if (Store.live && !confirm('Place a REAL phone call now? The AI agent will call ' + (r[t.nameKey] || 'this contact') + '.')) return;
    const lv = document.getElementById('cq-live'); if (lv) lv.classList.remove('hidden');
    await this._runCall(t, r);
    if (lv) lv.classList.add('hidden');
    this.show(tabId);
  },
  // Single entry point for "make a call for this row" so live and demo can never
  // be confused: live dials for real and waits for the server to record the
  // outcome; demo runs the scripted animation.
  _runCall(t, r) { return Store.live ? this._placeLiveCall(t, r) : this._simulateCall(t, r); },
  async _placeLiveCall(t, r) {
    const name = r[t.nameKey];
    const liveText = document.getElementById('cq-live-text');
    const stCell = document.getElementById('cq-st-' + r.id);
    if (liveText) liveText.textContent = 'Dialing ' + name + ' (' + (r[t.phoneKey] || '') + ')…';
    if (stCell) stCell.innerHTML = '<span class="px-2 py-0.5 rounded-md text-[11px] border font-medium badge-sky">📞 dialing…</span>';
    try { await t.liveCall(r, this); } catch (e) { toast('Call could not be placed', 'error'); }
    // Deliberately no status write here. The real outcome is decided on the
    // phone and recorded server-side; polling brings it back on its own.
    if (stCell) stCell.innerHTML = statusBadge(r[t.statusKey || 'status']);
    this._scheduleLiveRefresh();
    await this._sleep(900);
  },
  _sleep(ms) { return new Promise(res => setTimeout(res, ms)); },
  async _simulateCall(t, r) {
    if (Store.live) return;   // belt-and-braces: never fabricate against real records
  ...
  /* ------------------------------ BOARD --------------------------------- */
  /* A kitchen-display style column board: one column per status, cards move
   * across as staff advance them. Config:
   *   { type:'board', collection, columns:[{status,label,color,hint}],
   *     card:(r,A)=>html, actions:[{id,label,icon,color,when,run}],
   *     stats:(A)=>[{label,value,color}] }                                 */
  view_board(t) {
    const rows = Store.all(t.collection);
    const q = (this.filters[t.id + ':q'] || '').toLowerCase().trim();
    const shown = q && t.searchKeys
      ? rows.filter(r => t.searchKeys.some(k => String(r[k] || '').toLowerCase().includes(q)))
      : rows;
    const stats = t.stats ? `<div class="grid grid-cols-2 md:grid-cols-4 gap-3">${t.stats(this).map(s =>
      `<div class="border border-border rounded-xl bg-card p-4"><div class="text-xs text-muted-fg font-medium">${esc(s.label)}</div>
       <div class="text-xl font-semibold mt-1 ${s.color ? 'kpi-accent-' + s.color : 'text-fg'}">${s.value}</div>
       ${s.sub ? `<div class="text-[11px] text-muted-fg mt-0.5">${esc(s.sub)}</div>` : ''}</div>`).join('')}</div>` : '';

    const cols = (t.columns || []).map(c => {
      // Working columns read oldest-first — that is the order the kitchen should
      // work in. A column marked newestFirst (a closed stage like "Delivered
      // today") reads the other way, so that capping it with `limit` drops the
      // OLDEST cards rather than hiding the order that just completed.
      const dir = c.newestFirst ? -1 : 1;
      const all = shown.filter(r => r[t.statusKey || 'status'] === c.status && (!c.filter || c.filter(r)))
        .sort((a, b) => (b.priority ? 1 : 0) - (a.priority ? 1 : 0) ||
                        dir * String(a.created || '').localeCompare(String(b.created || '')));
      // Closed columns can grow all day; cap them so the board stays readable.
      const items = c.limit ? all.slice(0, c.limit) : all;
      const hidden = all.length - items.length;
      const cards = items.length ? items.map(r => {
        const acts = (t.actions || []).filter(a => !a.when || a.when(r)).map(a =>
          `<button onclick="event.stopPropagation();IndustryApp.runAction('${t.id}','${a.id}','${r.id}')" title="${esc(a.label)}"
             class="inline-flex items-center gap-1 px-2 py-1 rounded-md border border-border bg-card hover:bg-card-hover text-[11px] text-${a.color || 'muted'}-fg transition-colors">
             <i data-lucide="${a.icon}" class="w-3 h-3"></i>${esc(a.short || a.label)}</button>`).join('');
        return `<div class="border border-border rounded-lg bg-bg p-3 ${r.priority ? 'ring-1 ring-amber-fg/40' : ''} ${t.detail ? 'cursor-pointer' : ''}"
                     ${t.detail ? `onclick="IndustryApp.openDetail('${t.id}','${r.id}')"` : ''}>
          ${t.card ? t.card(r, this) : `<div class="text-sm text-fg">${esc(r.id)}</div>`}
          ${acts ? `<div class="flex flex-wrap gap-1.5 mt-2.5">${acts}</div>` : ''}
        </div>`;
      }).join('') : `<div class="text-[11px] text-muted-fg text-center py-8">${esc(c.hint || 'Nothing here')}</div>`;
      return `<div class="flex flex-col min-w-0">
        <div class="flex items-center justify-between px-1 pb-2">
          <div class="flex items-center gap-2">
            <span class="w-2 h-2 rounded-full" style="background:hsl(var(--${c.color || 'muted'}-fg))"></span>
            <h3 class="text-xs font-semibold uppercase tracking-wider text-muted-fg">${esc(c.label)}</h3>
          </div>
          <span class="text-[11px] text-muted-fg tabular-nums">${all.length}</span>
        </div>
        <div class="flex-1 rounded-xl bg-muted/40 border border-border p-2 space-y-2 min-h-[120px]">${cards}
          ${hidden > 0 ? `<div class="text-[11px] text-muted-fg text-center py-1.5">+${hidden} more — see the Orders tab</div>` : ''}</div>
      </div>`;
    }).join('');
    ...
    return `${stats}
      <div class="flex flex-wrap items-center gap-2">
        <div class="relative flex-1 min-w-[200px] max-w-xs">
          <i data-lucide="search" class="w-3.5 h-3.5 absolute left-2.5 top-1/2 -translate-y-1/2 text-muted-fg"></i>
          <input id="tbl-search" oninput="IndustryApp.onBoardSearch('${t.id}', this.value)" placeholder="Search orders…"
            class="w-full pl-8 pr-3 py-1.5 text-sm border border-border rounded-md bg-card text-fg" value="${esc(this.filters[t.id + ':q'] || '')}">
        </div>
        ${(t.bulk || []).map(b => `<button onclick="IndustryApp.runBulk('${t.id}','${b.id}')" class="text-sm px-3 py-1.5 rounded-md border border-border bg-card hover:bg-card-hover flex items-center gap-1.5 transition-colors"><i data-lucide="${b.icon || 'plus'}" class="w-3.5 h-3.5"></i>${esc(b.label)}</button>`).join('')}
        <span class="text-xs text-muted-fg ml-auto flex items-center gap-1.5">
          <span class="relative flex h-2 w-2"><span class="animate-ping absolute inline-flex h-full w-full rounded-full bg-emerald-fg opacity-60"></span><span class="relative inline-flex rounded-full h-2 w-2 bg-emerald-fg"></span></span>
          live</span>
      </div>
      <div class="grid gap-3 md:grid-cols-2 xl:grid-cols-4 items-start">${cols}</div>`;
  },
  onBoardSearch(tabId, v) {
    this.filters[tabId + ':q'] = v;
    const t = this.tab(tabId);
    document.getElementById('ind-view').innerHTML = this.view_board(t);
    if (window.lucide) lucide.createIcons();
    const inp = document.getElementById('tbl-search');
  ...
  view_reports(t) {
    ...
    const tables = (t.tables ? t.tables(this) : []).map(tb =>
      `<div class="border border-border rounded-xl bg-card overflow-hidden">
        ...
        <tbody>${tb.rows.map(row => `<tr class="border-t border-border">${row.map((c, i) => `<td class="px-5 py-2.5 ${i ? 'text-right font-mono text-xs' : 'text-fg'}">${esc(c)}</td>`).join('')}</tr>`).join('')}</tbody></table></div></div>`).join('');
  ...
  exportCurrent() {
    ...
    const csv = [keys.join(',')].concat(rows.map(r => keys.map(k => {
      let v = r[k]; if (v == null) v = ''; v = String(v);
      // Formula injection: a caller can dictate a free-text field (e.g. notes)
      // over the phone, and "=HYPERLINK(…)" would run as a live formula the
      // moment staff open the export in Excel/Sheets. A leading apostrophe
      // makes the spreadsheet treat the cell as text.
      if (/^[=+\-@]/.test(v)) v = "'" + v;
      v = v.replace(/"/g, '""');
      return /[",\n]/.test(v) ? `"${v}"` : v;
    }).join(','))).join('\n');
